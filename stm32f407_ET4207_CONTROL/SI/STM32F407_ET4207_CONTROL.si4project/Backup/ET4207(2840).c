...
u8 ET4207SendCode(u8 *etcode,int length){
	...
	u8 err=0;
	err = Hard_IIC_WriteNByte(I2C1,ET4207_ADDRESS,_ET4207_CONTROL_SEND_,length,etcode,&err);
	if(err!=0){
		return err;
		}
	err = Hard_IIC_WriteNByte(I2C1,ET4207_ADDRESS,_ET4207_CONTROL_SEND_END_,0,etcode,&err);
	if(err!=0){
		return err;
		}
	return err;
...
/**
  ...
  *****************************************************************************
**/
u8 Hard_IIC_PageRead(I2C_TypeDef* IICx, uint8_t SlaveAdd, u8 ReadAdd, u16 NumToRead, u8 * pBuffer)
{
	u16 i = 0;
	u16 temp = 0;
	u16 sta = 0;
