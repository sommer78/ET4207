...
//    n_flag = datas[10];
    ...
    *freq = 2500000 / n_Freq;
	...
	for(i=0;i<n_Sample;i++){
		   learn_buffer[n_PartIndexCount+n_Index+i] =datas[i+16+n_PartIndexCount+n_Index];
		 printf("n_Sample[%d] = 0x%02x \r\n", i,learn_buffer[n_PartIndexCount+n_Index+i] );
	   }
	if( data_len != n_PartIndexCount + n_Sample + n_Index ){
        printf("n length  error \r\n" );
        return 2;
    }
    if( n_Crc != xCal_crc(learn_buffer, data_len )){
        printf("n_Crc  error \r\n" );
        return 1;
    }
