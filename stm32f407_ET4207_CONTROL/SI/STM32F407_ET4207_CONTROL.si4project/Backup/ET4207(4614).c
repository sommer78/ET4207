...
//    n_flag = datas[10];
    ...
    *freq = 2500000 / n_Freq;
    ...
    for(i=0;i<n_PartIndexCount;i++){
       ...
       printf("partIndex[%d] = 0x%x", i,learn_buffer[i] );
    }
    for(i=0;i<n_Sample;i++){
        learn_buffer[n_PartIndexCount+i] =datas[i+16+n_PartIndexCount];
      printf("n_Sample[%d] = 0x%x", i,learn_buffer[n_PartIndexCount+i] );
    }

    for(i=0;i<n_Index;i++){
        learn_buffer[n_PartIndexCount+n_Sample+i] =datas[i+16+n_PartIndexCount+n_Sample];
       printf("index[%d] = 0x%x", i,learn_buffer[n_PartIndexCount+n_Sample+i] );
    }
